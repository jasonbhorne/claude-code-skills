---
name: new_calendar
description: Shift a GCS school calendar Excel file to the next school year, preserving all formatting. Rebuilds month grids with correct day-of-week alignment.
argument-hint: "[target_years, e.g. 2029-2030]"
---

Shift the most recent GCS school calendar to a new school year using the `new_calendar.py` script.
...
1. **Determine the source file.** If the user provides a file path, use it. Otherwise, find the most recent calendar:
   ```
   ls -t ~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Calendar/*/Board\ Proposal/*.xlsx | head -1
   ```
   If no Board Proposal exists, look in the year folder directly:
   ```
   ls -t ~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Calendar/*/*.xlsx | head -1
   ```
...
`~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Calendar/<YYYY-YYYY>/`
