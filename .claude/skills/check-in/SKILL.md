...
```
~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Meeting Notes/Friday Check-In/
```
...
- **Skip non-check-in files**: Capital Project Meeting.docx, Data Dashboard Preparation.docx, Electric Bus FAQ.docx, Electric Bus Points.docx, Friday Check.docx, Jason Horne.docx, Operations Team Meeting Notes Sept 6.docx
...
```python
DEPT_HEADS = {
    "debbie": "Debbie Collins - Tech Purchasing & Administration",
    "karen": "Karen Wilhoit - School Nutrition",
    "julie": "Julie Fannon - Data & EIS",
    "ellen": "Ellen Lipe - Finance",
    "melissa": "Melissa Batson - Human Resources",
    "phillip": "Phillip Graham - Maintenance & Facilities",
    "roger": "Roger Hensley - Custodial Services",
    "chuck": "Chuck Broyles - Technology",
    "tracie": "Tracie Valentine - Coordinated School Health",
    "kristen": "Kristen - Transportation & Operations",
}
```
...
Upload each cleaned DOCX to the "Friday Check-Ins" notebook so the full check-in history is searchable and available for podcast/report generation.

**Notebook ID:** `86dc3fa8-5bb7-4b7b-901f-c48a1c5555f9`

#### Skip logic

Before uploading, check existing sources to avoid duplicates:

```bash
notebooklm source list --notebook 86dc3fa8-5bb7-4b7b-901f-c48a1c5555f9 --json
```

Parse the JSON output. If a source with a title matching `Friday Check-Ins - YYYY-MM-DD` already exists, skip the upload.

#### Upload

```bash
notebooklm source add "path/to/Cleaned/Friday Check-Ins - YYYY-MM-DD.docx" --notebook 86dc3fa8-5bb7-4b7b-901f-c48a1c5555f9
```

Do NOT wait for source processing to complete (it happens in the background). Just confirm the upload was accepted and move on.

#### Batch upload

In batch mode (`--all`), upload all cleaned files at the end after all processing is complete. Check existing sources once, then upload only files not already present. Report the count: "Uploaded X new check-ins to NotebookLM (Y already present)."

#### Error handling

If the upload fails (auth expired, rate limit, etc.), log the error and continue. The local DOCX/PDF pipeline is the primary output; NotebookLM is supplementary. Suggest `notebooklm login` if auth fails.
