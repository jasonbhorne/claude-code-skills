...
description: Process Brandy's periodic Chronic Absenteeism Accountability spreadsheet for Greeneville City Schools. Produces an executive summary, per-principal packets, and a dashboard workbook, then archives to OneDrive and logs to Obsidian. Activate when the user says "/chronic-absenteeism", "chronic absenteeism snapshot", "process CA data", "CA spreadsheet", or mentions Brandy's absenteeism accountability file.
---
...
Brandy Parton (GCS attendance) periodically sends a multi-sheet Chronic Absenteeism Accountability workbook. This skill transforms that raw snapshot into a decision-ready set of deliverables: an executive summary for the C-Team, per-principal packets for each building leader, a pivoted dashboard workbook, and an Obsidian session log.

## Trigger

User says `/chronic-absenteeism`, "process chronic absenteeism", "run CA snapshot", "chronic absenteeism analysis", "Brandy's CA spreadsheet", or drops a file named like `Chronic Abs Accountability Data as of *.xlsx` into Downloads.
...
3. Ask the user for the instructional day count if you can't read it from the data (currently day 122 of 167, but this will drift). If every student row has the same `isp_days_to_date` and `instructional_calendar_days`, use those.
...
7. Recommended Follow-Ups: 5 bullets — distribute packets, SART/court sequence check, GHS triage, reconcile partial-year enrollees with Brandy, re-run in 4 weeks.
...
Target folder: `~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Data/Accountability Data/Chronic Absenteeism/`

Create the folder if it doesn't exist. Copy:
- The original source xlsx (preserves Brandy's red highlighting)
- The dashboard xlsx
...
Body sections:
- Task (what Brandy sent, what got produced)
...
- **Frequency**: Ad-hoc. Brandy sends these on her own cadence. After the first run in a month, subsequent pulls should be compared against the prior snapshot to measure movement (future enhancement: add a MoM tab).
- **No emoji, no emdashes, no bold body text** per Jason's global output style.

## Related
- First built 2026-04-20 from Brandy's 3/10/2026 snapshot. Session log: `Sessions/2026-04-20 - Chronic Absenteeism Snapshot 3-10.md`.
