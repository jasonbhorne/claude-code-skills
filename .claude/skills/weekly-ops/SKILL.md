...
3. `site-review`: Run the jasonhorne.org site review for last month.
4. `newsletter`: Generate the GCS AI Newsletter (due on the 27th, but can run anytime in the month).
...
Prompt the agent to:
1. Check `~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Meeting Notes/Friday Check-In/Raw/` for unprocessed .docx files (files without a corresponding cleaned version).
...
Prompt the agent to:
1. Read `~/Library/CloudStorage/OneDrive-GreenevilleCitySchools/Meeting Notes/Friday Check-In/Action Item Tracker.xlsx`
